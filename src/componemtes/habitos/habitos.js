...
export default function Habitos({ dados }) {
    ...
    function enviahabitos() {
        ...
        postHabitos(body,dados.token).then((ref) => {
            console.log(ref)
        })
        postHabitos(body).catch(() => { console.log(body); alert("Não foi possivel realizar o login,verifique seus dados e tente novamente!") })
